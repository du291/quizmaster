...
- **Work type:** migration
- **Current phase:** `Quiz.Take.Length` milestone complete; next feature slice ready
- **Known current state:**
  - Migration strategy is incremental.
  - WTR coverage exists for make-flow features plus `Quiz.Progress`, `Quiz.Score`, `Quiz.Score.Partial`, `Quiz.Take`, `Quiz.Take.Length`, `Quiz.Timer`, and `Quiz.Welcome`, with backend smoke companions.
  ...
  - The remaining helper-specific residual is now the narrower timer-proof question; the next migration-facing milestone can move to `Quiz.Bookmarks`.
...
- Whether `Quiz.Bookmarks` should stay feature-local in WTR or introduce a browser-storage / bookmark-state seam if the same persistence proof pattern repeats.
...
- New `Quiz.Bookmarks` mocked/backend coverage passes targeted checks in Chromium and Firefox.
- The command of record still passes after the milestone: `bash ./scripts/test-migration.sh`.
- Any new bookmark-state / persistence seam is either proven with executed evidence or recorded as **UNPROVEN** with the cheapest next proof.
- Residual helper concerns outside the bookmark slice remain explicit rather than silently expanding the milestone.
...
## 6) Milestones
### Current milestone
- **Name:** Migrate `Quiz.Bookmarks` in WTR lanes
- **Intent:** Add mocked and backend WTR coverage for bookmark behavior, especially question bookmarking and revisit flow, without widening scope unless storage-state proof demands it.
- **Entry conditions:**
  - `Quiz.Take.Length` is complete with targeted WTR evidence and a green command of record.
  - The command of record already encodes the proven Playwright worker setting.
- **Expected inventory / touched areas:**
  - `frontend/tests/wtr/mocked/quiz-bookmarks.test.tsx`
  - `frontend/tests/wtr/backend/quiz-bookmarks.backend.test.tsx`
  - shared WTR support helpers or a storage seam only if bookmark-state proof cannot stay feature-local
- **Planned evidence:**
  - targeted mocked/backend WTR runs for touched files
  - sampled rerun only if bookmark/state persistence shows race-sensitive behavior
  - full `bash ./scripts/test-migration.sh` before milestone close
- **Exit condition:**
  - `Quiz.Bookmarks` mocked/backend lanes are green with executed evidence, and any bookmark-state helper or seam residuals are explicitly recorded before moving to the next chosen slice.

### Upcoming milestones
1. Reassess whether the deferred timer-proof investigation or CI-policy decision needs to happen before the later migration batches.
2. Decide whether any slow legacy Playwright files should be split once migration pressure drops.
3. Choose the next remaining migration batch after `Quiz.Bookmarks`.
...
|---|---|---|---|---|---|---|
| R1 | `Quiz.Bookmarks` may need a browser-storage or bookmark-state seam beyond the current quiz-flow contracts | Could widen scope into infrastructure-seam work or leave persistence assertions too weak | Start feature-local first and reuse existing take-flow/state-proof helpers where possible | Medium | Targeted mocked/backend `Quiz.Bookmarks` runs after a narrow first implementation | Pull if the slice demands broader seam work |
